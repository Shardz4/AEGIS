...
impl PlumeEngine {
    ...
    pub fn compute(&self, params: &PlumeParams, zone_boundaries: &[ZoneBoundary]) -> PlumeResult {
        ...
        let max_concentration_ppm = self.compute_concentration_at(
            ...
            params.source_x
                + 1.0
                    * (((90.0 - ((params.wind_direction_deg + 180.0) % 360.0))
                        * std::f64::consts::PI
                        / 180.0)
                        .cos()),
            params.source_y
        ...
        for angle_deg in (0..360).step_by(10) {
            ...
            polygon.push((
                params.source_x + r_low * alpha.cos(),
                params.source_y + r_low * alpha.sin(),
            ));
        }
        ...
        for zone in zone_boundaries {
            ...
            let conc_at_center =
                self.compute_concentration_at(params, zone.center_x, zone.center_y);
...
mod tests {
    ...
    fn test_known_answer() {
        ...
        println!(
            "Computed concentration: {} ppm (expected ~2620.7 ppm)",
            conc
        );
        assert!(
            (conc - 2620.73).abs() < 5.0,
            "Expected ~2620.73, got {}",
            conc
        );
    }
    ...
    fn test_symmetry() {
        ...
        assert!(
            (conc_pos - conc_neg).abs() < 1e-10,
            "Expected perfect symmetry, got {} vs {}",
            conc_pos,
            conc_neg
        );
    }
    ...
    fn test_monotonicity() {
        ...
        assert!(
            conc_50 > conc_100,
            "Expected C(50m) > C(100m), got {} vs {}",
            conc_50,
            conc_100
        );
        assert!(
            conc_100 > conc_200,
            "Expected C(100m) > C(200m), got {} vs {}",
            conc_100,
            conc_200
        );
    }
    ...
    fn test_hazard_radius_dynamics() {
        ...
        assert!(
            ...
            res_high_q.hazard_radius_m,
            res_base.hazard_radius_m
        );
    ...
    fn test_performance() {
        ...
        assert!(
            elapsed.as_millis() < 5,
            "Plume computation should take < 5ms, took {:?}",
            elapsed
        );
